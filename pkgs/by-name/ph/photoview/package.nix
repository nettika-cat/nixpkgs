...
buildGoModule rec {
  ...
  postInstall = ''
    ...
    cp -r $src/api/data $out/share/photoview/

    # Symlink the UI
    ln -s ${passthru.ui} $out/share/photoview/ui
    ...
    wrapProgram $out/bin/photoview \
      ...
      --prefix PATH : ${lib.makeBinPath [ exiftool darktable ffmpeg ]}
  '';

  passthru.ui = callPackage ./ui.nix { inherit src version; };
