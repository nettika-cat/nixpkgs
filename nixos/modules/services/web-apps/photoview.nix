...

{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.photoview = {
      ...
      requires = lib.optional (cfg.database.type == "postgres") "postgresql.service"
        ++ lib.optional (cfg.database.type == "mysql") "mysql.service";
      ...
      environment = {
        ...
        PHOTOVIEW_DISABLE_FACE_RECOGNITION = if cfg.settings.disableFaceRecognition then "1" else "0";
        PHOTOVIEW_DISABLE_VIDEO_ENCODING = if cfg.settings.disableVideoEncoding then "1" else "0";
        PHOTOVIEW_DISABLE_RAW_PROCESSING = if cfg.settings.disableRawProcessing then "1" else "0";
        ...
        PHOTOVIEW_SQLITE_PATH = "${cfg.dataDir}/photoview.db";
      };
        ...
        let
          envSetup =
            if cfg.database.type == "mysql" then
              ''export PHOTOVIEW_MYSQL_URL="${cfg.database.user}:$(cat $CREDENTIALS_DIRECTORY/db_password)@tcp(${cfg.database.host}:${toString cfg.database.port})/${cfg.database.name}"''
            else if cfg.database.type == "postgres" then
              ''export PHOTOVIEW_POSTGRES_URL="postgres://${cfg.database.user}:$(cat $CREDENTIALS_DIRECTORY/db_password)@${cfg.database.host}:${toString cfg.database.port}/${cfg.database.name}"''
            else
              "";
        in
        ''
          ${envSetup}
          exec ${cfg.package}/bin/photoview
        '';
